...
ok("statut des références (Q88, Q93 actées) : ref_substitution_imprecis et ref_paires_chroniques exportables (agrégats seuillés, usage aval déclaré) ; v_admin_longs / v_admin_courts internes ; toute référence non marquée est interne par défaut ; REFS_EXPORTABLES inclus dans NOMS_REFS ; statuts_refs = liste nommée prête pour le méta",
   all(statut_ref(c("ref_substitution_imprecis", "ref_paires_chroniques")) == "exportable") && all(statut_ref(c("ref_v_admin_longs", "ref_v_admin_courts", "ref_pivots_courts", "ref_inconnue")) == "interne") && all(REFS_EXPORTABLES %in% NOMS_REFS) &&
     sum(statut_ref(NOMS_REFS) == "exportable") == 2 && identical(names(statuts_refs(NOMS_REFS)), NOMS_REFS) && statuts_refs(NOMS_REFS)[["ref_paires_chroniques"]] == "exportable" && is.list(statuts_refs(NOMS_REFS)))
