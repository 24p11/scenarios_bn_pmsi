###############################################################################
# demo/executer_notebook.R — exécute un notebook (RUN.Rmd / RUN_aval.Rmd) en MODE DÉMO, chunk par chunk
#
# Usage (racine du dépôt) : Rscript demo/executer_notebook.R [--raz] RUN.Rmd
#   --raz : vide demo/resultats/ avant (SCENARIOS_PMSI_DEMO_RAZ). Enchaînement CI : --raz RUN.Rmd, puis RUN_aval.Rmd.
...
# (malgré son eval=FALSE, c'est le mode démo) ; chunks portant l'option demo=FALSE sautés (non pertinents
# hors plateforme : migration, palier, vidages, tests) ; tout autre chunk exécuté, la première erreur arrête
# (code de sortie non nul). View() est remplacé par un head(), les graphiques vont dans un pdf nul.
...
local({
  ...
  cat("#### ", fichier, " : ", length(chunks), " chunks (mode démo)\n", sep = "")
