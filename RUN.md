...
| `etape_habillage_longs(populations = …)` | tirage | `40_campagnes/<C>/habille/<population>/lot_*.parquet` (jointure `ref_v_admin_longs.parquet` relu par lots sur les **6 clés** `CLES_ADMIN_LONGS` — nbda retiré —, photographie filtrée sur `DUREE_LONGS`, **repli hiérarchique** âge → cage → mode_hospit × cage × racine, variantes tirées au poids des effectifs `n`, colonne `repli_admin`, stop nominatif si aucun candidat ; DPEC/TPEC recalculés) | après un jeu de chunks complet (stop sinon) | réécrit les lots |
...
- Habillage admin des longs : 6 clés (`CLES_ADMIN_LONGS`, nbda retiré), repli hiérarchique, variantes tirées au poids
  des effectifs `n` de la photographie (aux replis, `n` sommés), colonne `repli_admin` (0 / 1 / 2) jusqu'au corpus,
  contrôles « zéro NA » et « durée dans le périmètre de la branche » au rapport, stop nominatif sinon — jamais de NA
  silencieux ; un scénario ne reçoit jamais la durée de l'autre branche.
